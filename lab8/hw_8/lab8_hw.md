...
```
## here() starts at /Users/tiffanysmacbookpro/Desktop/BIS15W2023_tchan
```
...
The data is tidy since each variable has its own column and each observation has its own row. The column names are also not data. And every column has data instead of variables.
...
```r
average_enterocci<-sydneybeaches_dates %>% 
  group_by(year) %>% 
  summarize("average_enterocci"=mean(enterococci_cfu_100ml,na.rm=T))
average_enterocci
```

```
## # A tibble: 6 × 2
##   year  average_enterocci
##   <chr>             <dbl>
## 1 2013               50.6
## 2 2014               26.3
## 3 2015               31.2
...
```r
average_enterocci_wide <- average_enterocci %>% 
  pivot_wider(names_from="year",
              values_from="average_enterocci")
average_enterocci_wide
```
...
sydneybeaches_dates %>% 
  ...
  group_by(site) %>% 
  summarize("average_enterocci"=mean(enterococci_cfu_100ml,na.rm=T)) %>% 
  arrange(desc(average_enterocci)) %>% 
  head(n=1)
```

```
## # A tibble: 1 × 2
##   site                    average_enterocci
##   <chr>                               <dbl>
