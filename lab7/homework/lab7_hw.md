...
```
## [1] "/Users/tiffanysmacbookpro/Desktop/BIS15W2023_tchan/lab7/homework"
```
...
These values do not make sense since there are multiple -999 in the data. NAs are represented by the value -999
...
In this data set the values make sense since the NAs are represented as NAs. They are not values.
...
```r
amphibio %>% 
  summarize(ter_number_nas=sum(is.na(Ter)),
            fos_number_nas=sum(is.na(Fos)),
            arb_number_nas=sum(is.na(Arb)),
            aqu_number_nas=sum(is.na(Aqu)))
```

```
## # A tibble: 1 × 4
##   ter_number_nas fos_number_nas arb_number_nas aqu_number_nas
##            <int>          <int>          <int>          <int>
## 1           1104           6053           4347           2810
```


I do not think the authors meant that the data had NAs but instead the NAs meant to show that the animal did not live fossorial, terrestrial, aquatically, or arboreally. They are supposed to be shown as a quality like true or false.
...
```r
amniota_altered<-readr::read_csv("data/amniota.csv",na=("-999"))
```
