...
homerange %>% mutate_if(is.character, as.factor)
  #shift+command+M creates t" %>% =pipe" 
```
...
```{r}
fish %>%
  select(lakeid, scalelength) %>% #don't need to put fish in as data table
  filter(lakeid == "AL") 
#the results of one function will be feed into another function
```
...
mammals %>% 
  ...
  filter(gestation>=15)
#use more than one function in an r coding block
```
