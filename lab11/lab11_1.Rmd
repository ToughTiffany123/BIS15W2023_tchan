...
- [Rebecca Barter `ggplot` #####Tutorial](http://www.rebeccabarter.com/blog/2017-11-17-ggplot2_tutorial/)#IMPORTANT
...
```{r}
homerange %>% 
  ggplot(aes(x=class,fill=class,y=log10.mass))+
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="deepskyblue4", alpha  =0.4, color = "steelblue2")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(fill="snow",color="steelblue2")+
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_density(fill="snow",color="steelblue2")+
  labs(title="Distribution of Mass")
```
...
homerange %>% 
  ...
  select(class,range_category,log10.mass) %>% 
  ggplot(aes(x=class,fill=class,y=log10.mass))+
  geom_boxplot(alpha=0.5,fill="steelblue2")
```
