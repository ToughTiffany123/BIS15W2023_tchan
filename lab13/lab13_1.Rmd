...
List of all csv files in found after path: data/spiders
```{r}
...
lapply applies a function to a vector which contains all of the csv files
```{r, message = FALSE}
...
inner_join only gave what they ahd in common but removed the things that were different between the two data tables.
```{r}
...
left_join kept the customer id of table_A removed table B that did not align.
```{r}
...
Kept the customer id of table_B and removed any customer if that wasn't in table_B
```{r}
...
`full_join`
kept all of the customer id's from both table_A and table_B
```{r}
...
Shows what was different in the second table
```{r}
anti_exampleDF <- anti_join(table_A, table_B, by="customer_ID")
anti_exampleDF
```
```{r}
anti_exampleDF2 <- anti_join(table_B, table_A, by="customer_ID")
anti_exampleDF2
```
