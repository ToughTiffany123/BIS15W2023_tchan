...
Shows everything but Alabama Lake
...
```{r}
limitted<-select(mammals,"family","genus","species","mass","gestation")
limitted
```

5. Use the output from #4 to focus on the family Bovidae with a mass greater than 450000.
```{r}
big_bovidae<-filter(limitted,family == "Bovidae" & mass>450000)
big_bovidae
```
...
lem<-filter(mammals,genus=="Macaca"|genus == "Lophocebus"|genus=="Erythrocebus")
lem
```
```{r}
lophocebus
...
```
```{r}
mammals_2<-select(lem,"mass","gestation","newborn")
mammals_2
```
```
